...
function clears() {
    ...
    localStorage.removeItem("selectedIndexes3");
    populateTable()
}
function loadSelection() {
    var storedIndexes3 = localStorage.getItem("selectedIndexes3");
    return storedIndexes3 ? JSON.parse(storedIndexes3) : [];
}
function getCheckedRecord() {
    var count3 =0
    var table = document.getElementById('table3');
    var selectedIndexes3 = [];
    for (var i = 0; i < table.rows.length; i++) {
        var checkbox = table.rows[i].cells[0].querySelector('input[type="checkbox"]');
        if (checkbox.checked) {
            selectedIndexes3[count3] = i;
            count3++
        }
    }
    localStorage.setItem("count3", JSON.stringify(count3));
    localStorage.setItem("selectedIndexes3", JSON.stringify(selectedIndexes3));
}
function populateTable() {
    ...
    var selectedIndexes3 = loadSelection();
    courseData.forEach(function(course, index3) {
        ...
        if (selectedIndexes3.includes(index3)) {
...
window.onload = function() {
    populateTable();
    document.querySelector('#table3').addEventListener('change', getCheckedRecord);
};
function myFunction() {
    // Declare variables
